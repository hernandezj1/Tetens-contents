...
<xsl:stylesheet version="2.0"
    ...
    <!-- ===================================================== -->
    <!-- REMOVED: standalone pb template (cannot group content) -->
    <!-- ===================================================== -->
    <!--
    ...
    <xsl:template match="/">
        <html>
            ...
            <body>
                <div id="text">
                    <!-- CHANGED: body is now handled by its own template -->
    ...
    <!-- ===================================================== -->
    <!-- NEW: Page grouping logic (pb + following content)     -->
    <!-- ===================================================== -->
            ...
            <xsl:choose>
                <xsl:when test="self::tei:pb">
                    <div class="page" data-page="{@n}">
                        <xsl:apply-templates
                            select="current-group()[not(self::tei:pb)]"/>
                    </div>
                </xsl:when>
                <xsl:otherwise>
                    <!-- content before first pb -->
    ...
    <xsl:template match="tei:lb">
        <xsl:choose>
            <xsl:when test="preceding-sibling::text()[last()][matches(., '-$')]
                            and following-sibling::text()[1]">
